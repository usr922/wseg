...
class VOC12ImageDataset(Dataset):
    ...
    def __init__(self, img_name_list_path, voc12_root, transform=None, salpath=None):
        self.img_name_list = load_img_name_list(img_name_list_path)
        self.voc12_root = voc12_root
        self.transform = transform
        self.salpath = salpath
    ...
    def __getitem__(self, idx):
        ...
        # open salency
        if self.salpath is not None:
            sal_map = PIL.Image.open(os.path.join(self.salpath, name + '.png'))
            # bg_map = np.where(sal_map <= 0.5 * 100, 1, 0)
            if self.transform:
                sal_map = self.transform(sal_map)
                # TODO: 这里显著性map也要跟着同步变换，这里先不管，后续没办法了再上显著性
        if self.transform:
            img = self.transform(img)

        if self.salpath is not None:
            return name, img, bg_map
        else:
            return name, img
...
class VOC12ClsDataset(VOC12ImageDataset):
    ...
    def __init__(self, img_name_list_path, voc12_root, transform=None, salpath=None):
        super().__init__(img_name_list_path, voc12_root, transform, salpath)
    ...
    def __getitem__(self, idx):
        if self.salpath is not None:
            name, img, bg_map = super().__getitem__(idx)
        else:
            name, img = super().__getitem__(idx)

        label = torch.from_numpy(self.label_list[idx])
        if self.salpath is not None:
            return name, img, label, bg_map
        else:
